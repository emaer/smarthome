...
//	private String url;
	...
	public void setChildren(ArrayList<OpenHABWidget> children) {
		this.children = children;
	}

	public void setMappings(ArrayList<OpenHABWidgetMapping> mappings) {
		...
		if (startNode.hasChildNodes()) {
			...
			for (int i = 0; i < childNodes.getLength(); i ++) {
				...
				if (childNode.getNodeName().equals("item")) {
					...
					if (childNode.getNodeName().equals("widgetId")) {
						this.widgetId = childNode.getTextContent();
	...
	public String toString() {
		...
		String str = "OpenHABWidget [label=" + label + ", icon=" + icon + ", type="
				+ type + ", refresh=" + refresh + ", linkedPage=" + linkedPage;
		
		if(null != item)
			str += ",item=" + item.toString();
		
		if(mappings.size() != 0)
			str += getMappingsString();
		
		if(null != children && children.size() != 0)
			str += getChildrenString();
